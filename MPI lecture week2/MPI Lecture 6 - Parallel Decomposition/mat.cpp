...
// A * B = C, A -> i_max by k_max, B -> k_max by j_max and C -> i_max by j_max
...
void setup_continuous_array(double** &array_2d, double*& array_1d, int m, int n)
{
     ...
     for (int i = 0; i < m; ++i)
         array_2d[i] = &array_1d[i * n];              // index_ij = i * n + j
}
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < i_max * k_max; ++i)
        A_1d[i] = ((double)rand()) / ((double)RAND_MAX) * 10;

    for (int i = 0; i < k_max * j_max; ++i)
        B_1d[i] = ((double)rand()) / ((double)RAND_MAX) * 10;
