...
void create_communication_list(void)
{
	...
	//Note that this can be better done using the collective communication MPI_Alltoall, which we will learn about next lecture
	for (int i = 0; i < p; i++)
	{
		if (i != id)
		{
			MPI_Irecv(&receive_list[i], 1, MPI_C_BOOL, i, tag_num, MPI_COMM_WORLD, &request[cnt]);
			cnt++;
			MPI_Isend(&send_list[i], 1, MPI_C_BOOL, i, tag_num, MPI_COMM_WORLD, &request[cnt]);
			cnt++;
		}
		else
...
void do_communication_round(int com_round)
{
	...
	for (int i = 0; i < p; i++)
	{
		if (send_list[i])
		{
			cout << "communication round: " << com_round << ": " << id << " process sent " << data_to_send[sent] << " to process " << i << endl;
			cout.flush();
			sent++;
		}
		...
		if (receive_list[i])
		{
			cout << "communication round: " << com_round << ": " << id << " process received " << data_to_receive[received] << " from process " << i << endl;
			cout.flush();
			received++;
		}
	}
